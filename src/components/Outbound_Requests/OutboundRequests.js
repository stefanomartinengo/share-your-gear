...
getRequests() {
    axios.get("/outbound/requests/" + this.props.user.userid).then(response => {
    this.setState({
...
render() {
    var sentRequests = this.state.requests.map((e, i) => {
    if (e.pending === true) {
        return (
    ...
    <div>
        ...
        <div className="main-container">
            ...
            <div className="list-subcontainer">{sentRequests}</div>
        ...
        <h1>
            {" "}
            PLEASE <Link to="/"> SIGN IN </Link> TO VIEW YOUR REQUESTS{" "}
        </h1>
        )}
    </div>
    );
